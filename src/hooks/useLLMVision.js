...
import { AutoTokenizer, RawImage, Tensor } from "@huggingface/transformers"
...
class Qwen2VLBackend {
    ...
    async generate(prompt, systemPrompt, callbacks, { image }) {
...
export function useLLMVisionGeneration({
    ...
    const generateText = React.useCallback(
        async (prompt) => {
            ...
            await generate(fullPrompt, { image: image });
        },
        [generate]
    );
